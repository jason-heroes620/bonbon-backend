  ...
  const onRegisterSubmit = async (data: RegisterFormValues) => {
    ...
    try {
        ...
        if (error.response && error.response.data && error.response.data.errors) {
             const errors = error.response.data.errors;
             if (errors.email) {
                 registerForm.setError('email', { message: errors.email[0] });
             }
             if (errors.password) {
                 registerForm.setError('password', { message: errors.password[0] });
             }
        } else if (error.response && error.response.data && error.response.data.message) {
             setServerError(error.response.data.message);
        } else {
      ...
      <div className="hidden lg:flex lg:w-1/2 relative bg-indigo-600 overflow-hidden">
        ...
        <div className="relative w-full flex flex-col justify-between p-12 text-white">
          <div className="flex items-center space-x-2">
            ...
            <span className="text-xl font-bold tracking-wider">BONBON</span>
          </div>
          <div className="space-y-6 max-w-lg">
            ...
            <p className="text-indigo-100 text-lg">
              Experience the sweet taste of productivity. Manage your projects, collaborate with your team, and achieve your goals with Bonbon.
            </p>
          </div>
        ...
        <div className="w-full max-w-md space-y-8">
          ...
          <div className="mt-8">
              ...
              <form onSubmit={registerForm.handleSubmit(onRegisterSubmit)} className="space-y-6">
                <div className="space-y-5">
                  ...
                  <div className="p-4 bg-gray-50 rounded-lg space-y-2 border border-gray-100">
                    <p className="text-xs font-medium text-gray-500 uppercase tracking-wide mb-2">Password Requirements</p>
                    <div className="grid grid-cols-1 gap-2">
                        <PasswordRequirement label="At least 8 characters" met={registerForm.watch('password')?.length >= 8} />
                        <PasswordRequirement label="Contains uppercase & lowercase" met={/[A-Z]/.test(registerForm.watch('password') || '') && /[a-z]/.test(registerForm.watch('password') || '')} />
                        <PasswordRequirement label="Contains number & special char" met={/[0-9]/.test(registerForm.watch('password') || '') && /[^A-Za-z0-9]/.test(registerForm.watch('password') || '')} />
                    </div>
                  </div>
                  ...
                  <div className="flex items-start">
                    ...
                    <div className="ml-3 text-sm">
                      ...
                      {registerForm.formState.errors.terms && (
                        <p className="mt-1 text-sm text-red-600">{registerForm.formState.errors.terms.message}</p>
                      )}
                    </div>
                  </div>
                </div>
            ...
            <div className="mt-6 grid grid-cols-2 gap-3">
              ...
              <SocialButton icon="https://www.svgrepo.com/show/503173/apple-logo.svg" label="Apple" />
            </div>
          </div>
        </div>
      </div>
...
function InputField({ id, label, type, placeholder, registration, error }: any) {
  return (
    <div>
...
function PasswordRequirement({ label, met }: { label: string; met: boolean }) {
  return (
    <div className={cn("flex items-center text-xs transition-colors duration-200", met ? "text-green-600 font-medium" : "text-gray-400")}>
      <div className={cn("w-4 h-4 rounded-full flex items-center justify-center mr-2 border", met ? "bg-green-100 border-green-200" : "bg-gray-100 border-gray-200")}>
