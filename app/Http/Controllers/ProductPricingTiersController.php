...
class ProductPricingTiersController extends Controller
{
    ...
    public function update(Request $request, Products $product, ProductPricingTier $tier)
    {
        if ((string) $tier->product_id !== (string) $product->product_id) {
            return response()->json([
        ...
        $existingMode = ProductPricingTier::query()
            ->where('product_id', $product->product_id)
            ->where('is_active', true)
            ->where('product_pricing_tier_id', '!=', $tier->product_pricing_tier_id)
            ->value('pricing_mode');
    ...
    public function destroy(Request $request, Products $product, ProductPricingTier $tier)
    {
        if ((string) $tier->product_id !== (string) $product->product_id) {
            return response()->json([
                'message' => 'Pricing tier not found.',
